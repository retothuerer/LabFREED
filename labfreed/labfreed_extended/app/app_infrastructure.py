    ...
    def process_pac(self, pac_url) -> PacInfo:
        ...
        for sg in service_groups:  
            ...
            for url in attributes_urls:
                ags = {ag.group_key: AttributeGroup.from_attribute_group(ag) for ag in self._attribute_client.get_attributes(url, subject_id=pac.to_url(include_extensions=False), language_preferences=self._language_preferences)}
                if ags:
