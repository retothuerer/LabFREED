...
def callback_factory():
    ...
    # a clean skip.
    from labfreed.pac_attributes.client.client import authenticated_http_attribute_request_callback_factory
...
def test_matching_rule_header_is_applied_to_the_actual_request(callback_factory, rules, session_and_adapter):
...
def test_non_matching_rule_leaves_request_unauthenticated(callback_factory, rules, session_and_adapter):
    session, adapter = session_and_adapter
    callback = callback_factory(rules, session=session)
    request_data = AttributeRequestData(pac_id="HTTPS://PAC.EXAMPLE.COM/1")
